-- Fix All Notification Functions and Constraints (SIMPLIFIED)
...
-- 9. Test the system with a simple follow operation (NO RETURNING id)
DO $$
DECLARE
  test_user1_id uuid;
  test_user2_id uuid;
BEGIN
  ...
  IF test_user1_id IS NOT NULL AND test_user2_id IS NOT NULL AND test_user1_id != test_user2_id THEN
    -- Try to create a test follow (NO RETURNING id)
    ...
    -- Clean up test follows (NO id column)

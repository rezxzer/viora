-- Complete migration to fix streams status column type
-- This approach drops dependent views, converts column, then recreates views
...
-- 3) Safely convert status column step by step
DO $$
BEGIN
...
-- 6) Recreate the streams_with_url_v2 view
...
DO $$
BEGIN
  RAISE NOTICE 'Migration completed successfully! Status column converted to stream_status enum and views recreated.';
END$$;

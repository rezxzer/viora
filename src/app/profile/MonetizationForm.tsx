...
const planSchema = z.object({
  ...
  currency: z.string().min(3).max(3).transform((v) => v.toLowerCase()),
...
export default function MonetizationForm({ userId }: Props) {
  ...
  const [plan, setPlan] = useState<Plan>({ name: "Supporter", price_cents: 500, currency: "usd", is_active: true });
    ...
    const load = async () => {
      ...
      try {
        const [{ data: settings }, { data: planRow }]: [
          ...
          supabase.from("creator_settings").select("is_monetized").eq("user_id", userId).maybeSingle(),
          supabase.from("subscription_plans").select("id, name, price_cents, currency, is_active").eq("creator_id", userId).eq("is_active", true).maybeSingle(),
        ]);
        if (!cancelled) {
        ...
        try {
          const { data: earn }: { data: { creator_id: string; total_gross_cents: number; total_creator_cents: number; total_platform_cents?: number } | null } = await supabase
        ...
        try {
          const { data: subsCount }: { data: { creator_id: string; active_subscribers: number } | null } = await supabase
  ...
  const fmt = new Intl.NumberFormat(undefined, { style: "currency", currency: plan.currency || "usd" });
  const planDisabled = !enabled;
  const payoutCents = Math.max((totalTipsCents || 0) - (totalPlatformCents || 0), 0);
  const platformPct = (totalTipsCents || 0) > 0 ? Math.min(100, Math.max(0, (totalPlatformCents / totalTipsCents) * 100)) : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border bg-surface p-4 shadow-soft">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <div className="text-sm text-muted-foreground">Allow subscriptions and tips for your profile.</div>
          </div>
          <input
            type="checkbox"
      ...
      <div className={"rounded-2xl border bg-surface p-4 shadow-soft space-y-3 " + (planDisabled ? "opacity-60 pointer-events-none" : "") }>
        <div className="font-medium">Monthly plan</div>
        <div className="grid gap-3 md:grid-cols-3">
          <Input placeholder="Plan name" value={plan.name} onChange={(e) => setPlan({ ...plan, name: e.target.value })} disabled={planDisabled} />
          <Input
        ...
        <div className="flex gap-2">
          <Button onClick={onSavePlan} disabled={savingPlan || planDisabled}>{savingPlan ? "Saving..." : "Save plan"}</Button>
          {plan.id ? (
            <Button variant="destructive" onClick={onDeletePlan} disabled={savingPlan || planDisabled}>
              Disable
            </Button>
          ) : null}
        </div>
          ...
          <>
            <div className="grid grid-cols-3 gap-6">
              <div>
                ...
                <div className="mt-1 text-xs text-muted-foreground">Platform fees: {fmt.format((totalPlatformCents || 0) / 100)}</div>
              </div>
              <div>
                <div className="text-xl font-semibold">{fmt.format((totalTipsCents || 0) / 100)}</div>
            ...
            {totalTipsCents === 0 && activeSubscribersCount === 0 ? (
              <div className="mt-3 text-sm text-muted-foreground">No earnings yet — share your profile link to get started.</div>
            ) : null}
          </>
        )}
      </div>

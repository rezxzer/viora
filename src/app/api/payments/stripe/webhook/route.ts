  ...
  try {
    ...
    if (!stripeSecret || !webhookSecret || !sig) {
      ...
      if (event.type === "checkout.session.completed") {
        ...
        const pi = (event.data.object as Stripe.PaymentIntent) || ({} as Partial<Stripe.PaymentIntent>);
        session = {
          id: pi.id as string | undefined,
          payment_status: "paid",
          amount_total: typeof pi.amount_received === "number" ? (pi.amount_received as number) : null,

...
export default async function FeedPage({ searchParams }: { searchParams?: Promise<{ after?: string }> }) {
  ...
  const [
    ...
    postIds.length
      ? supabase
          ...
          .in("post_id", postIds)
      : Promise.resolve({ data: [] as Array<{ post_id: string; likes_count: number; comments_count: number }>, error: null as null }),
    ...
    authorIds.length
      ? supabase
          ...
          .in("id", authorIds)
      : Promise.resolve({ data: [] as Array<{ id: string; username: string | null; full_name: string | null; avatar_url: string | null }>, error: null as null }),
    userId
      ? supabase.from("profiles").select("id, username, full_name, avatar_url").eq("id", userId).maybeSingle()
      : Promise.resolve({ data: null, error: null } as { data: { id: string; username: string | null; full_name: string | null; avatar_url: string | null } | null; error: null }),
    postIds.length
      ? supabase.from("post_assets").select("post_id, url, mime_type").in("post_id", postIds)
      : Promise.resolve({ data: [] as Array<{ post_id: string; url: string; mime_type: string }>, error: null as null }),
  ]);

  const postIdToStats = new Map<string, { likes_count: number; comments_count: number }>();
  const statsRows = (stats ?? []) as Array<{ post_id: string; likes_count: number; comments_count: number }>;
  statsRows.forEach((s) => postIdToStats.set(s.post_id, { likes_count: s.likes_count, comments_count: s.comments_count }));

  const likedRows = (liked ?? []) as Array<{ post_id: string }>;
  const likedSet = new Set<string>(likedRows.map((r) => r.post_id));
  const authorMap = new Map<string, { username: string | null; full_name: string | null; avatar_url: string | null }>();
  const authorRows = (authors ?? []) as Array<{ id: string; username: string | null; full_name: string | null; avatar_url: string | null }>;
  authorRows.forEach((a) => authorMap.set(a.id, { username: a.username, full_name: a.full_name, avatar_url: a.avatar_url }));
  ...
  return (
    <div className="mx-auto w-full max-w-[720px] space-y-4">
      {userId ? (
        <Composer
          userId={userId}
          avatarUrl={(viewerProfileResult as { data: { avatar_url: string | null } | null } | null)?.data?.avatar_url ?? authorMap.get(userId)?.avatar_url ?? null}
        />
      ) : null}

...
export default function NotificationBell() {
  ...
  const enrichWithActors = async (items: NotificationRow[]): Promise<NotificationRow[]> => {
    ...
    const byId = new Map<string, { id: string; username: string | null; full_name: string | null; avatar_url: string | null }>();
    for (const a of actors as any[]) {
      byId.set(a.id, { id: a.id, username: a.username ?? null, full_name: a.full_name ?? null, avatar_url: a.avatar_url ?? null });
    }
    return items.map((i) => {
  ...
  return (
    <div className="relative">
      <Button size="sm" variant={unread ? "default" : "outline"} onClick={() => setOpen((v) => !v)} aria-label="Notifications">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 rounded-xl border bg-surface p-2 shadow-soft z-50">
          <div className="flex items-center justify-between mb-2">
            ...
            <button className="text-xs underline" onClick={markRead} disabled={unread === 0}>Mark all read</button>
          </div>
          {loading ? (
                ...
                <li key={r.id} className={`rounded-lg p-2 ${r.read_at ? "opacity-70" : "bg-elev"}`}>
                  <div className="flex items-start gap-2">
                    <Avatar className="h-8 w-8">
                      <AvatarImage src={r.actor_avatar ?? undefined} alt={r.actor_name ?? r.actor_username ?? "user"} />
                      <AvatarFallback>{(r.actor_username ?? "U").slice(0, 1).toUpperCase()}</AvatarFallback>
                    </Avatar>
                    <div className="min-w-0">
                      <div className="text-sm">
                        {r.actor_username ? (
                          <Link href={`/u/${r.actor_username}`} className="font-medium hover:underline">

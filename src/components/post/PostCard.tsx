...
export default function PostCard({
  ...
  const [recentComments, setRecentComments] = useState<
    Array<{ id: string; author_id: string; content: string; created_at: string; author: { name: string; avatar_url: string | null } }>
  >([]);
    ...
    const loadRecent = async () => {
      try {
        const { data: rows, error } = await supabase
          ...
          .limit(2);
        if (error) return;
        const list = (rows ?? []) as Array<{ id: string; author_id: string; content: string; created_at: string }>;
        if (list.length === 0) {
          setRecentComments([]);
          return;
        }
        ...
        (profs ?? []).forEach((p: { id: string; username: string | null; full_name: string | null; avatar_url: string | null }) => {
          map.set(p.id, { name: p.full_name || p.username || "User", avatar_url: p.avatar_url ?? null });
        });
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            setRecentComments((prev) => [...prev, { id: r.id, author_id: r.author_id, content: r.content, created_at: r.created_at, author: prev.find(p => p.author_id===r.author_id)?.author ?? { name: 'User', avatar_url: null } }].slice(-2));
            setCommentsCount((v) => v + 1);
          } else if (payload.eventType === 'UPDATE') {
            const r = payload.new as { id: string; content: string };
            setRecentComments((prev) => prev.map((c) => (c.id === r.id ? { ...c, content: r.content } : c)));
  ...
  const imageAssets = (assets || []).filter((a) =>
    a && (a.mime_type?.startsWith("image/") || /\.(png|jpe?g|gif|webp|avif|svg)(?:\?.*)?$/i.test(a.url))
  );
  const videoAssets = (assets || []).filter((a) =>
    a && (a.mime_type?.startsWith("video/") || /\.(mp4|webm|ogg)(?:\?.*)?$/i.test(a.url))
  );
  ...
  const onPrev = () => setLightboxIndex((i) => (i - 1 + displayImageUrls.length) % displayImageUrls.length);
    ...
    <Card className="p-4">
      <div className="flex gap-3">
        <Link href={authorUsername ? `/u/${authorUsername}` : `#`} className="shrink-0">
          <Avatar>
            {authorAvatarUrl ? <AvatarImage src={authorAvatarUrl} alt="avatar" /> : <AvatarFallback>U</AvatarFallback>}
          </Avatar>
        </Link>
        <div className="flex-1 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 rounded-xl overflow-hidden border">
                {displayImageUrls.slice(0, 4).map((url, idx) => {
                  ...
                  return (
                    ...
                    >
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={url} alt="media" className="absolute inset-0 h-full w-full object-cover" />
                    </button>
                  );
                })}
              </div>
                ...
                <div key={c.id} className="flex items-start gap-2 text-sm">
                  ...
                  <div className="flex-1">
                    <div className="text-xs text-muted-foreground">
                      <span className="font-medium text-foreground">{c.author.name}</span> · {new Date(c.created_at).toLocaleString()}
                    </div>
                    <div className="whitespace-pre-wrap break-words">{c.content}</div>
                  </div>
                </div>
              ))}
              <button className="text-xs text-muted-foreground hover:underline" onClick={() => setCommentsOpen(true)}>
                View all comments
              </button>
            </div>
          ) : null}
          ...
          {displayImageUrls.length > 0 ? (
            <Dialog open={lightboxOpen} onOpenChange={setLightboxOpen}>
              <DialogContent className="max-w-5xl p-0 bg-transparent border-0 shadow-none" showCloseButton>
          ...
          <div className="flex items-center justify-between pt-2">
            <div className="flex items-center gap-3">
              <Button variant={liked ? "secondary" : "outline"} size="sm" onClick={onToggleLike} disabled={isPending}>

...
export default function CommentsDialog({ postId, open, onOpenChange, viewerId, onCommentAdded, postAuthorId = null }: Props) {
  ...
  const [paidStats, setPaidStats] = useState<Map<string, { viora_count: number; viora_amount_cents: number }>>(new Map());
    ...
    try {
      ...
      if (authorIds.length) {
        ...
        if (postAuthorId && viewerId === postAuthorId) {
          ...
          (blocks ?? []).forEach((b: { blocked_id: string; expires_at: string | null }) => bmap.set(b.blocked_id, { expires_at: b.expires_at }));
          setBlockMap(bmap);
        } else {
          setBlockMap(new Map());
        }
      ...
      if (commentIds.length) {
        ...
        (stats ?? []).forEach((s: { comment_id: string; viora_count: number; viora_amount_cents: number }) => {
          sMap.set(s.comment_id, { viora_count: s.viora_count ?? 0, viora_amount_cents: s.viora_amount_cents ?? 0 });
        });
        setPaidStats(sMap);
      } else {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            const row = payload.new as CommentRow;
            setRows((prev) => prev.map((r) => (r.id === row.id ? { ...r, content: row.content } : r)));
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            setReactions((prev) => {
              ...
              current.set(row.comment_id, { counts: { ...state.counts }, mine: new Set(state.mine) });
              return current;
            });
          } else if (payload.eventType === 'DELETE') {
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            const row = payload.new as { comment_id: string; reaction_code: string; amount_cents: number };
            if (row.reaction_code !== 'VIORA') return;
            setPaidStats((prev) => {
              const m = new Map(prev);
              const cur = m.get(row.comment_id) ?? { viora_count: 0, viora_amount_cents: 0 };
              m.set(row.comment_id, { viora_count: cur.viora_count + 1, viora_amount_cents: cur.viora_amount_cents + (row.amount_cents || 0) });
              return m;
            });
          } else if (payload.eventType === 'DELETE') {
            const row = payload.old as { comment_id: string; reaction_code: string; amount_cents: number };
            if (row.reaction_code !== 'VIORA') return;
            setPaidStats((prev) => {
              const m = new Map(prev);
              const cur = m.get(row.comment_id) ?? { viora_count: 0, viora_amount_cents: 0 };
              m.set(row.comment_id, { viora_count: Math.max(0, cur.viora_count - 1), viora_amount_cents: Math.max(0, cur.viora_amount_cents - (row.amount_cents || 0)) });
              return m;
            });
          }
        }
  ...
  const onSubmit = () => {
    ...
    startTransition(async () => {
      try {
        ...
        const { data, error } = await supabase
          .from("post_comments")
          .insert({ post_id: postId, author_id: viewerId, content, parent_id: optimistic.parent_id })
          .select("id, post_id, author_id, content, created_at, parent_id")
          .single();
        if (error) throw error;
  ...
  const blockUser = async (targetUserId: string, duration: "24h" | "7d" | "30d" | "permanent") => {
    ...
    try {
      const { error } = await supabase
        .from("user_blocks")
        .upsert({ blocker_id: postAuthorId, blocked_id: targetUserId, expires_at: expires }, { onConflict: "blocker_id,blocked_id" });
      if (error) throw error;
  ...
  const toggleReaction = async (commentId: string, emoji: string) => {
    ...
    const next: ReactionState = {
      counts: { ...state.counts, [emoji]: Math.max(0, (state.counts[emoji] ?? 0) + (has ? -1 : 1)) },
      mine: new Set(Array.from(state.mine.values()).filter((e) => e !== emoji).concat(has ? [] : [emoji])),
    };
  ...
  const rendered = useMemo(() => {
    ...
    const renderOne = (r: CommentRow, depth: number) => {
      ...
      return (
        <div key={r.id} className="flex items-start gap-2 py-2" style={{ marginLeft: depth ? 28 : 0 }}>
          <Avatar className="h-8 w-8">
            {p?.avatar_url ? <AvatarImage src={p.avatar_url} alt="avatar" /> : <AvatarFallback>U</AvatarFallback>}
          </Avatar>
          <div className="flex-1 min-w-0">
            <div className="text-xs text-muted-foreground">
              <span className="font-medium text-foreground">{name}</span> · {new Date(r.created_at).toLocaleString()}
            </div>
            {editing?.id === r.id ? (
              <div className="space-y-2">
                <Textarea value={editing.value} onChange={(e) => setEditing({ id: r.id, value: e.target.value })} className="min-h-16" />
            ...
            <div className="mt-1 flex items-center gap-2 text-xs text-muted-foreground">
              ...
              >
                ...
                <svg viewBox="0 0 24 24" className="inline h-4 w-4 align-[-2px] mr-1" aria-hidden="true">
                  <defs>
                  ...
                  <circle cx="12" cy="12" r="9" fill="none" stroke="url(#vioraGrad)" strokeWidth="1.8"/>
                  <path d="M7.5 8l4.5 8 4.5-8" fill="none" stroke="url(#vioraGrad)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                {paid.viora_count}
              </button>
              {!viewerBlocked && (
              ...
              {canEdit ? (
                <button type="button" className="px-2 py-0.5 rounded hover:bg-elev" onClick={() => startEdit(r)}>
                  Edit
                </button>
              ) : null}
              {canDelete ? (
                <button type="button" className="px-2 py-0.5 rounded hover:bg-elev text-red-400" onClick={() => removeComment(r)}>
                  Delete
                </button>
              ) : null}
              {canModerate ? (
                blockedInfo ? (
                  <>
                    <span className="ml-2 inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] text-muted-foreground">Blocked</span>
                    <button type="button" className="px-2 py-0.5 rounded hover:bg-elev" onClick={() => unblockUser(r.author_id)}>
                      Unblock
                    </button>
                  </>
                ) : (
                  <div className="relative inline-flex items-center gap-1">
                    <span>Block:</span>
                    <button type="button" className="px-2 py-0.5 rounded hover:bg-elev" onClick={() => blockUser(r.author_id, "24h")}>24h</button>
                    <button type="button" className="px-2 py-0.5 rounded hover:bg-elev" onClick={() => blockUser(r.author_id, "7d")}>7d</button>
                    <button type="button" className="px-2 py-0.5 rounded hover:bg-elev" onClick={() => blockUser(r.author_id, "30d")}>30d</button>
                    <button type="button" className="px-2 py-0.5 rounded hover:bg-elev text-red-300" onClick={() => blockUser(r.author_id, "permanent")}>Permanent</button>
                  </div>
                )
              ) : null}
            </div>
      ...
      <DialogContent className="max-w-xl">
        ...
        {replyTo ? (
          <div className="text-xs text-muted-foreground -mt-1">
            Replying to <span className="font-medium text-foreground">{profiles.get(replyTo.author_id)?.full_name || profiles.get(replyTo.author_id)?.username || "User"}</span>
            <button className="ml-2 underline" onClick={() => setReplyTo(null)}>cancel</button>
          </div>
        ) : null}
        {viewerBlocked ? (

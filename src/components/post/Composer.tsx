...
export default function Composer({ userId, avatarUrl }: Props) {
  ...
  const onFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const f of validIncoming) if (!byKey.has(`${f.name}_${f.size}`)) byKey.set(`${f.name}_${f.size}`, f);
    ...
    startTransition(async () => {
      try {
        ...
        if (files.length > 0) {
          ...
          // Insert assets rows
          const rows = uploaded.map((u) => ({ post_id: created.id, url: u.url, mime_type: u.mime_type }));
      ...
      <CardContent className="pt-6">
        <div
          ...
          onDrop={(e) => {
            ...
            if (dtFiles.length) {
              const inputLike = { target: { files: dtFiles } } as unknown as React.ChangeEvent<HTMLInputElement>;
              onFile(inputLike);
            }
          }}
        >
          ...
          <div className="flex-1">
            ...
            <div className="mt-3 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <input ref={fileRef} type="file" className="hidden" onChange={onFile} accept="image/*,video/*" multiple />
              ...
              <Button onClick={onPost} disabled={isPending || (!text && files.length === 0)} aria-label="Post">
            ...
            {previews.length > 0 ? (
              <div className="mt-3 grid grid-cols-2 gap-2">
                {previews.map((url, idx) => {
                  ...
                  return (
                    <div key={url + idx} className="relative aspect-square overflow-hidden rounded-lg border">
                      {isImage ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={url} alt="preview" className="absolute inset-0 h-full w-full object-cover" />
                      ) : (
